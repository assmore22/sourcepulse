...
import { writeMethod, waitAccepted, busyMessage, hasContract } from "@/lib/sourcepulse";
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ToastCtx.Provider value={{ push, update }}>
      {children}
      <div className="pointer-events-none fixed bottom-20 right-4 z-[60] flex w-[min(92vw,360px)] flex-col gap-2">
        {toasts.map((t) => (
          <div key={t.id} className="panel pointer-events-auto flex items-start gap-3 p-3 text-sm shadow-panel animate-fadeUp">
            <FontAwesomeIcon
              icon={t.kind === "ok" ? faCircleCheck : t.kind === "error" ? faCircleExclamation : faSpinner}
              className={`mt-0.5 h-4 w-4 ${t.kind === "ok" ? "text-primary" : t.kind === "error" ? "text-danger" : "animate-spin text-cyan"}`}
            />
            <div className="flex-1">
              <div className="font-mono font-semibold text-text">{t.title}</div>
              {t.msg && <div className="mt-0.5 text-xs text-muted">{t.msg}</div>}
              {t.hash && (
                <a href={explorerTx(t.hash)} target="_blank" rel="noreferrer" className="mt-1 inline-flex items-center gap-1 text-xs text-cyan hover:underline">
                  <span className="mono">{truncateHex(t.hash, 8, 6)}</span><FontAwesomeIcon icon={faArrowUpRightFromSquare} className="h-2.5 w-2.5" />
                </a>
              )}
            </div>
            <button type="button" onClick={() => dismiss(t.id)} className="text-muted hover:text-text"><FontAwesomeIcon icon={faXmark} className="h-3.5 w-3.5" /></button>
          </div>
        ))}
      </div>
    </ToastCtx.Provider>
